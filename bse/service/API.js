...
var DAILY_STOCKS_CLOSING_HEADERS =
  require('../constant').DAILY_STOCKS_CLOSING_HEADERS;
...
function getIndexStocks(symbolKey) {
  return axios({
    ...
    transformResponse: [
      function (data) {
        var actualData = data.split('$#$');
        var stocks = _.compact(
          _.map(actualData[1].split('|'), function (stock) {
    ...
    transformResponse: function (responseData) {
      try {
        ...
        var data = responseData
          ...
          .replace(/((<([^>]+)>)|&nbsp;|\(|\)|%)/gi, ''); // remove html tags and extra garbage chars
...
function getStocksChartData(securityCode, flag) {
  ...
  return axiosTransformerAdvance(
    ...
    DAILY_STOCKS_CLOSING_HEADERS,
    DAILY_STOCKS_HEADERS
  );
}

function getStockInfoAndDayChartData(securityCode) {
  return axiosTransformerAdvance(
    DAILY_STOCKS_URL + securityCode,
    DAILY_STOCKS_CLOSING_HEADERS,
    DAILY_STOCKS_HEADERS
  );
}
